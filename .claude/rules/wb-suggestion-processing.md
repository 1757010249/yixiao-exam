...
2. 按 `knowledge-maintenance.md` 的"全量检验"要求，逐条**先回查讲义 OCR 原文**（优先 `tools/extracted-vision/` 视觉版）确认内容一致，不得凭记忆或推断
   - **核查顺序**：先讲义 → 讲义有该内容且与建议一致，直接判定；讲义没有该内容、或讲义与建议/规范冲突 → **先询问用户**，经用户同意后才可联网（curl/WebSearch）查规范原文。禁止跳过讲义直接联网
   - 讲义与规范冲突时，以讲义为准记录并标注差异，不得擅自按规范修正
3. 逐条判定：**接受** / **拒绝** / **需用户确认**
