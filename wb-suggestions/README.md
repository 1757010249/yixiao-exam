# wb-suggestions — WorkBuddy 建议回流目录

> 本目录是 **WorkBuddy（wb）** 与 **Claude Code** 之间的协作接缝。
> wb 在问答中发现知识库盲区/疑误时，只在这里**新建**建议文件；Claude Code 认领、校验后决定是否写入知识库。
...
- wb 写建议后告知用户"已提交建议，将由 Claude Code 校验"。**wb 不得自己写入 knowledge-base/ 或更新 index.md**。
